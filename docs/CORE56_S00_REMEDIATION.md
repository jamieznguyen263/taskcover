...
- **Full repo suite**: `npm test` → 247/247 passing across 47 files. `npm run lint` clean. `npx tsc --noEmit` clean.
- `npm run build`: compiles and type-checks successfully; static generation then intermittently times out fetching a handful of `/insights/[categorySlug]/[articleSlug]` pages from the live database (60s-per-page retry budget exceeded for ~1 of 325 static pages under this session's network conditions to the live DB). This is **environmental, not caused by this PR** — none of the changed files (§1) are in the import graph of the insights page, its `generateStaticParams`, or the content provider; `INSIGHTS_PROVIDER=database` in `.env.local` is what drives full DB-backed static generation of all real published articles (including the 56 heavy Core 56 ones) at build time, independent of which branch is checked out. Not claiming a clean `npm run build` pass with certainty; see the PR for the latest retry result.
